...
#  edited by Mr Riaz Hussain Zehri
...
################################################################################
GENERAL_CONFIG_FILE = 'login.txt'

fp = open(GENERAL_CONFIG_FILE, 'r')
GENERAL_CONFIG = eval(fp.read())
fp.close()

LOCAL_CONFIG_FILE = 'modules/xmpp/local.txt'

fp = open(LOCAL_CONFIG_FILE, 'r')
LOCAL_CONFIG = eval(fp.read())
fp.close()
...
AFFILIATIONS={'none':0, 'member':11, 'admin':15, 'owner':35}
...
BOT_VER = {'rev': 13, 'botver': {'name': 'Riax-bot', 'ver': '4.5-%s', 'os': ''}}
################################################################################
...
def find_plugins():
	...
	if invalid_plugins:
		...
		print invp
		print 'plug-ins ARE NOT SUPPORTED BY SYSTEM PLEASE CONTACT RIAZ HUSAAIN 03063473312\n'
	else:
		print '\nALL plug-ins ARE UNLOADABLE'
...
def change_access_perm_glob(source, level=0):
	global GLOBACCESS
	jid = get_true_jid(source)
	temp_access = eval(read_file(GUYRO_FILE))
	if level:
		temp_access[jid] = level
	else:
		del temp_access[jid]
	write_file(GUYRO_FILE, str(temp_access))
	get_access_levels()
...
#	load_plugins()
	...
	print '\n..--==RIAZ CHAT BOT IS READY TO ROCK==--..\n'
...
if __name__ == "__main__":
	try:
		...
		JCON.send(prs)
		time.sleep(2)
		print '..--==RIAX CHAT BOT DISCONNECTED==--..'
		print '\n..--==RIAX CHAT BOT STOPPED==--..\n'
		sys.exit(0)
	except:
		if AUTO_RESTART:
